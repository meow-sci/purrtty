# purrTTY Terminal Emulator Game Mod
...
1. **Build the Mod** (if building from source):
   ```bash
   cd purrTTY-ksa/purrTTY.GameMod
   dotnet build
   ```

2. **Deploy the Mod**:
    - Copy all files from `purrTTY-ksa/purrTTY.GameMod/dist/` to your KSA mods directory
    - The mod files include:
        - `purrTTY.GameMod.dll` - Main mod assembly
        - `purrTTY.GameMod.deps.json` - Dependency information
...
Look for messages prefixed with "purrTTY GameMod:" in the game console.
...
dotnet build purrTTY-ksa/purrTTY-cs.sln

# Or build just the game mod
dotnet build purrTTY-ksa/purrTTY.GameMod/purrTTY.GameMod.csproj
```
...
├── purrTTY.GameMod.csproj    # Project file with KSA references
├── mod.toml                # Mod metadata
...
This project is part of the purrTTY terminal emulator implementation.
