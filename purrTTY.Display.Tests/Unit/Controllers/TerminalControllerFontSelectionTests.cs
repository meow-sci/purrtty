...
namespace purrTTY.Display.Tests.Unit.Controllers;

/// <summary>
/// Unit tests for font selection functionality supporting TerminalController.
/// Tests PurrTTYFontManager methods used by font selection UI.
...
public class TerminalControllerFontSelectionTests
{
    ...
    {
        // Act
        var availableFonts = PurrTTYFontManager.GetAvailableFontFamilies();
    ...
    public void CreateFontConfigForFamily_WithValidFontFamily_ShouldReturnValidConfig()
    {
        ...
        // Act
        var config = PurrTTYFontManager.CreateFontConfigForFamily(fontFamily, fontSize);
    ...
    public void GetCurrentFontFamily_WithKnownFontConfig_ShouldReturnCorrectFamily()
    {
        ...
        var config = PurrTTYFontManager.CreateFontConfigForFamily(expectedFamily, 14.0f);
    ...
    public void CreateFontConfigForFamily_WithInvalidFontFamily_ShouldHandleGracefully()
    {
        ...
        Assert.DoesNotThrow(() => 
        {
            var config = PurrTTYFontManager.CreateFontConfigForFamily(invalidFontFamily, fontSize);
    ...
    public void CreateFontConfigForFamily_WithNullFontFamily_ShouldHandleGracefully()
    {
        ...
        Assert.DoesNotThrow(() => 
        {
            var config = PurrTTYFontManager.CreateFontConfigForFamily(nullFontFamily!, fontSize);
            // If it returns a config, it should be valid (uses default font size 32.0f)
            if (config != null)
            {
    ...
    public void CreateFontConfigForFamily_WithEmptyFontFamily_ShouldHandleGracefully()
    {
        ...
        Assert.DoesNotThrow(() => 
        {
            var config = PurrTTYFontManager.CreateFontConfigForFamily(emptyFontFamily, fontSize);
            // If it returns a config, it should be valid (uses default font size 32.0f)
            if (config != null)
            {
    ...
    {
        // Arrange
        var validFontFamilies = PurrTTYFontManager.GetAvailableFontFamilies();
        var fontSize = 14.0f;
    ...
    public void GetCurrentFontFamily_WithUnknownFontConfig_ShouldReturnNull()
    {
        ...
        // Act
        var detectedFamily = PurrTTYFontManager.GetCurrentFontFamily(unknownConfig);
    ...
    public void CreateFontConfigForFamily_WithInvalidFontSize_ShouldPreserveInvalidSize()
    {
        ...
        foreach (var invalidSize in invalidSizes)
        {
            ...
            Assert.DoesNotThrow(() => 
            {
                var config = PurrTTYFontManager.CreateFontConfigForFamily(fontFamily, invalidSize);
        ...
        foreach (var specialSize in specialSizes)
        {
            Assert.DoesNotThrow(() => 
            {
                var config = PurrTTYFontManager.CreateFontConfigForFamily(fontFamily, specialSize);
    ...
    public void FontSelection_RoundTrip_ShouldMaintainConsistency()
    {
        ...
        {
            // Act
            var config = PurrTTYFontManager.CreateFontConfigForFamily(originalFamily, fontSize);
            var detectedFamily = PurrTTYFontManager.GetCurrentFontFamily(config);
    ...
    {
        // Arrange
        var initialFonts = PurrTTYFontManager.GetAvailableFontFamilies();
        
        // Act
        PurrTTYFontManager.LoadFonts(); // Call again
        var fontsAfterReload = PurrTTYFontManager.GetAvailableFontFamilies();
