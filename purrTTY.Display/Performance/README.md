...
The purrTTY terminal emulator includes a built-in high-precision performance measurement system for analyzing ImGui rendering performance. This system uses QueryPerformanceCounter (via `Stopwatch.GetTimestamp()`) to provide microsecond-level precision timing data for all rendering operations.
