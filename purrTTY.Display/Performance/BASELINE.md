...
This document provides instructions for collecting baseline performance metrics for the purrTTY terminal emulator in the KSA game environment. These baselines establish expected performance characteristics and help identify regressions or optimization opportunities.

## Prerequisites

1. **Game Build**: Ensure you have the latest build of purrTTY.GameMod
   ```bash
...
2. **Mod Installation**: Copy the mod to your KSA mods folder
   ```bash
   # The build outputs to: purrTTY.GameMod/bin/Debug/net10.0/purrTTY.dll
   # Copy contents to: C:\Program Files\Kitten Space Agency\Mods\purrTTY\
   ```
...
```
purrTTY.Display/Performance/Baselines/
